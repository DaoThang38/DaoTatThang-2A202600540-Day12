...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for part in splits:
            attempt = current_chunk + sep + part if current_chunk else part
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        chunks_fc = fc.chunk(text)
        result['fixed_size'] = {
            'count': len(chunks_fc),
            'avg_length': sum(len(c) for c in chunks_fc) / len(chunks_fc) if chunks_fc else 0,
            'chunks': chunks_fc
        }
        
        sc = SentenceChunker(max_sentences_per_chunk=3)
        chunks_sc = sc.chunk(text)
        result['by_sentences'] = {
            'count': len(chunks_sc),
            'avg_length': sum(len(c) for c in chunks_sc) / len(chunks_sc) if chunks_sc else 0,
            'chunks': chunks_sc
        }
        
        rc = RecursiveChunker(chunk_size=chunk_size)
        chunks_rc = rc.chunk(text)
        result['recursive'] = {
            'count': len(chunks_rc),
            'avg_length': sum(len(c) for c in chunks_rc) / len(chunks_rc) if chunks_rc else 0,
            'chunks': chunks_rc
        }
